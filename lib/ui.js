...
function printBrand(painter, version, subtitle = 'Repository workflow, refined.') {
  const name = 'X C L O N E';
  const versionLabel = `v${version}`;
  const width = 44;
  const gap = ' '.repeat(Math.max(1, width - name.length - versionLabel.length - 4));
  const subtitleText = `  ${subtitle}`.padEnd(width, ' ');

  console.log('');
  console.log(`  ${painter.violet('╭' + '─'.repeat(width) + '╮')}`);
  console.log(
    `  ${painter.violet('│')}  ${painter.bold(painter.white(name))}${gap}${painter.accent(versionLabel)}  ${painter.violet('│')}`
  );
...
function printInfo(painter, label, value) {
  console.log(`  ${painter.dim(label.padEnd(11, ' '))} ${value}`);
}
...
function printResultCard(painter, title, lines) {
  const width = 52;
  console.log('');
  console.log(`  ${painter.success('╭' + '─'.repeat(width) + '╮')}`);
  console.log(`  ${painter.success('│')}  ${painter.bold(title.padEnd(width - 2, ' '))}${painter.success('│')}`);
  for (const line of lines) {
    const clipped = String(line).slice(0, width - 2);
    console.log(`  ${painter.success('│')}  ${clipped.padEnd(width - 2, ' ')}${painter.success('│')}`);
  }
  console.log(`  ${painter.success('╰' + '─'.repeat(width) + '╯')}`);
}
